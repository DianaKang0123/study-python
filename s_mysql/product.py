...
if __name__ == '__main__':
    ...
    update_query = "update tbl_product set price = price * (1-(0.01*%s)) where price >= %s"
    ...
    delete_query = ("delete from tbl_product \
                    where price > (select avg.average from \
                    (select avg(price) average from tbl_product) avg)")
