...
if __name__ == '__main__':
    ...
    # # 회원가입
    # message = "이메일: "                                                 # 회원가입할 이메일 입력 메세지
    # client_email = input(message)                                       # 회원 가입 이메일 입력받기
    # # 아이디(이메일) 중복검사
    # find_by_id_query = "select email from tbl_client where email = %s"  # 클라이언트 테이블에서 입력받은 이메일이 있는지 확인하기
    # find_by_id_params = client_email,                                   # 입력받은 이메일을 찾는 이메일에 입력
    # result = find_by_id(find_by_id_query, find_by_id_params)            # pk로 찾은 결과를 result에 담아줌
    #
    # if not result:                                                      # result가 false라면: 찾은 값이 없다면 새로운 이메일
    #     message = "비밀번호: "                                            # 사용할 비밀 번호 입력 메세지
    #     client_password = input(message)                                # 사용할 비밀번호 입력받기
    #     encryption = hashlib.sha256()                                   # hashlib를 통한 비밀번호 암호화
    #     encryption.update(client_password.encode('utf-8'))              # 입력받은 pw를 업데이트로 암호화
    #     client_password = encryption.hexdigest()                        # 16진수로 변경해서 다시 담아주기
    #
    #     message = "이름: "                                               # 회원이름 입력 메세지
    #     client_name = input(message)                                    # 회원 이름 입력 받기
    #     save_query = "insert into tbl_client(email, password, name) values(%s, %s, %s)" # 회원 정보를 저장할 테이블 쿼리
    #     save_params = client_email, client_password, client_name        # 입력받은 이메일, 비번, 이름을 각 칼럼에 저장
    #     save(save_query, save_params)                                   # 저장 함수
    #     print(f"{client_name}님 환영합니다~!")                             # 완료를 나타내는 메세지 출력
    # else:                                                               # 만약 result 가 true(입력한 값이 이미 테이블에 존재)
    #     print("이미 사용중인 이메일입니다.")                                 #  오류 메세지 출력

    # # 회사 추가
    # save_many_query = "insert into tbl_office (name, location) \
    #                    values(%s, %s)"                                 # 오피스 테이블에 이름과 장소에 내용 추가
    # save_many_params = (
    #     ('네이버', '서울'),
    #     ('카카오', '판교'),
    #     ('구글', '종로')
    # )                                                                   # 추가 할 값
    # save_many(save_many_query, save_many_params)                        # 추가함수
    #
    # # 회의실 추가
    # find_by_id_query = "select id from tbl_office where id = %s"        # 각 오피스의 아이디를 찾기
    # find_by_id_params = 3,                                              # 오피스의 아이디
    # office = find_by_id(find_by_id_query, find_by_id_params)            # 찾은 아이디의 정보를 오피스에 담아주기
    # office_id = office.get('id')                                        # 딕셔너리에서 id 값 추출
    #
    # save_many_query = "insert into tbl_conference_room(office_id) values (%s)"  # 회의실의 오피스 아이디에 해당 아이디를 저장
    # save_many_params = office_id                                        # 추출한 아이디
    # save(save_many_query, save_many_params)                             # 저장함수
    #
    # # 회의실마다 이용가능 시간 추가
    # find_by_id_query = "select * from tbl_conference_room where id = %s"    # 회의실에 id를 이용하여 찾기
    # find_by_id_params = 4,                                              # 회의실 아이디
    # conference_room = find_by_id(find_by_id_query, find_by_id_params)   # 회의실 정보를 찾아 conf room에 담아주기
    # conference_room_id = conference_room.get('id')                      # 딕셔너리에서 id 를 찾아서 추출하기
    #
    # save_many_query = "insert into tbl_part_time(time,conference_room_id) values(%s,%s)"    # 시간 테이블에서 값 넣기
    # save_many_params = (
    ...
    # )                                                               # 시간 값
    # save_many(save_many_query, save_many_params)                    # 시간 넣어주는 함수
    #
    # 예약 추가
    message = '이메일: '
        ...
        if client_password == client.get('password'):
            ...
            if part_time.status:
                ...
                save_query = "insert into tbl_reservation(time, client_email, conference_room_id) \
                              values (%s, %s, %s)"
                save_params = part_time.__str__(), client.email, conference_room.id
                save(save_query, save_params)
